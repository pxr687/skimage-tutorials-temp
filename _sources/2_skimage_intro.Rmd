...
The solution here is to `np.stack()` the `camera` image, to introduce colour channels. We then pass our 3-D array to `ski.util.random_noise()`, manipulating the `var` argument to control the "level" of the noise. Specifically, `var` will control the range of the changes. Higher values of `var` means bigger absolute random values can potentially be added/subtracted from each pixel:
...
Let's extract the individual channels with some array indexing operations. Each channel is 2-D, and therefore will render as a greyscale image when displayed with Matplotlib:
...
# Not all 3-D images are in colour...

We noted [earlier](1_colour_images_as_numpy_arrays) that it is important to know that not all 3-D images contain colour information. What sort of images are these? Typically, they are images obtained during [medical imaging](https://en.wikipedia.org/wiki/Medical_imaging) scans, like brain imaging. The "slices" in the third dimension, for these images, are literally slices of a 3-D object (like a brain, chest, arm or leg!) rather than colour channels. 
...
- Some 3-D images are volumetric, and do not contain colour channels, despite having a third dimension. `skimage` supports these image formats as well.
