...
This page will cover another local filer - the median filter. The median filter differs in important ways from the local filters from [previous](5_mean_filter) pages, because it cannot be implemented through convolution.
...
The median filter is especially useful for removing noise from images, while preserving the "edges" in the image. Let's look at why this is the case, by expanding the `nums` array into a low-resolution image, using `np.tile()` and `.reshape()`.
