...
the same process of "walking" through the image with a small array which
defines the "local neighborhood" of each pixel.
...
However, instead of *applying a kernel* to the neighborhood array and the
image, as we would for a mean filter or Gaussian filter etc., the median
filter, you may be surprised to hear, takes the *median* value of the pixels
under the footprint.

![](images/footprint_general.png)

There is no way of using a kernel to calculate the median, and this is why
we and [others](https://dsp.stackexchange.com/questions/13211/convolution-kernels-for-image-filtering)
will tell you not to call the footprint a "kernel", when we are doing median
filtering. Calculating the median utilizes ranking and indexing, and cannot be
expressed as set of additions, subtractions, multiplications, divisions etc,
in the way that calculating a mean can, for instance.
...
There is no kernel method that can do this - instead we use the footprint to
...
a *kernel filter* because it does not use a weighted weighted sum of the local
...
This process is shown below, for three footprints ("local
neighborhoods" under a 3-by-3 array). The central pixel is highlighted in red,
and the index of the current local neighborhood is shown above the pixel
values.
...
Because the median is not a function which can be applied via kernel
filtering, we cannot apply it using `ndi.correlate()`, as we did with the
other [local filters](5_mean_filter).

We can apply it in `skimage` using `ski.filters.median()`. Again,  we supply
a `footprint` argument to determine the size and shape of each pixel's "local
neighborhood":
...
Whilst the median filter does not use a kernel, it is still a local filter, because it filters pixels based on the values of other pixels in the local neighborhood. Other filters use neither kernel filtering nor a local pixel neighborhood. These filters are called *non-local filters*. 
...
This page has shown a local filter (the median filter) and a non-local filter (histogram equalisation) which do not use kernel filtering to perform their filtering operations. On the [next page](8_morphology) we will introduce another method for modifying pixels based on a `footprint`.
