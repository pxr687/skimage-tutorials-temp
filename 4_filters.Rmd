...
**NOTE: change threshold line to dashed line**
<!-- #endregion -->
...
**NOTE: Amend to: As you can see, the OTSU calculation finds a threshold such that the pixel values on each side of the threshold are clustered closely around their respective means.  In fact, it turns out this calculation also has the effect that …**
...
**NOTE - introduce this via emphasizing Skimage compatibility with scientific python ecosystem e.g. integrated with Matplotlib**

```{python}
ski.filters.try_all_threshold(camera, verbose=False, figsize=(16, 16))
plt.tight_layout();
```
