...
You can think of it as finding a threshold which groups pixels such that the pixel values in each side of the "split" are clustered closely around their respective mean. In fact, it turns out minimizing the total `ssd` also has the effect that the *distance* between the means will be large as well giving a big separation between the pixels in each class as a result.
...
A useful trick, when thresholding in `skimage` to try to separate foreground and background, is to use the `ski.filters.try_all_threshold()` to try all the available thresholding methods, and visually inspect the results.

This function is both a filtering function and a plotting function. Behind the scenes it is using Matplotlib to generate the plots. Such is the utility of `skimage` being highly integrate with the rest of the [Scientific Python eco-system](https://scientific-python.org).
...
We got a blurrier output image via `skimage` here, than in the manual convolution using `ndi.correlate()` because we used a larger kernel for the `skimage` filtering, and so are averaging over larger pixel neighbourhoods.

Scikit-image will alos handle this using a 3D kernel. Below we adjust our `footprint_9_by_9` kernel by `np.stack()`-ing it into 3D:
...
You'll notice that the colours have changed substantially. This is because the 3D kernel is leading to averaging across the colour channels, relative to filtering each channel separately. We may want this effect (it is somewhat retro), but if we do not want to affect the colour of the image, then generally filtering the channels separately is the way to go...
