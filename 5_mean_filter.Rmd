...
## Kernels and pixel neighborhoods
...
a "kernel", "footprint" or "structuring element". As we will see later, Scikit
image favors the term "footprint". For now, we will use the terms "kernel" and
...
a kernel/neighborhood. To see this in action, let's build a simple mean filter
...
Let's use some indexing to go for a walk through the neighborhoods (kernels) in
...
Now, imagine we calculate the mean within each neighborhood (kernel) - the mean of each kernel is shown below the image of each indexing operation:
...
The cell below defines a function that will apply this mean kernel, step-by-step in a for loop to the whole image array, only for pixel neighborhoods where the kernel "fits" over the central pixel, without extending outside of the image:
